...
import { getgender } from '@/api/data'  // 导入资金信息相关接口
...
const data = {
  ...
  mutations: {
    SET_DIALOG_TITLE: (state, type) => {
      if(type === 'add'){
        state.addFundDialog.title = '新增信息'
        state.addFundDialog.type = 'add'
      }else{
        state.addFundDialog.title = '编辑信息'
  ...
  actions: {
     // 获取资金列表
     Getgender({commit},reqData) {
        return new Promise(resolve => {
            getgender(reqData).then(response => {
